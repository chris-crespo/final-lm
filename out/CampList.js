...
const CampList = ({
  ...
  useEffect(() => {
    fetch(`${api}/camps`).then(res => res.text()).then(text => new window.DOMParser().parseFromString(text, "text/xml")).then(xml => Array.from(xml.firstChild.children).map(parseCamp)).then(camps => {
      setCamps(camps);
  ...
  const parseCamp = camp => Array.from(camp.children).reduce((acc, child) => ({ ...acc,
    [child.localName]: child.localName === "langs" ? parseLangs(child.children) : child.textContent
  }), {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    class: "camps-cards"
  }, filteredCamps.map(camp => /*#__PURE__*/React.createElement(CampCard, {
    camp: camp
  }))));
};
